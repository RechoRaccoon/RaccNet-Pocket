 ...
 * quantization. Ported from legacy util/GifEncoder.kt — adapted from the
 * classic public-domain implementations by Anthony Dekker (NeuQuant) and
 * Kevin Weiner (AnimatedGifEncoder).
 *
 ...
 *
 * Internal to the platform layer; shared code goes through [encodeGif].
 */
internal class GifEncoder(private val out: OutputStream) {
    ...
    fun addFrame(bitmap: android.graphics.Bitmap, highQuality: Boolean = false) {
        if (!started) return
        if (width == 0) {
            width = bitmap.width
