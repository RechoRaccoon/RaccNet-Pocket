 ...
 *  keyed by post URI, built by [TaggingRepository]
 ...
 *
 *  The [ExportedPost]/[DatasetInfo] transfer types now live in common code
 *  (com.mediaviewer.platform) so the shared [TaggingController] contract can
 *  use them; this class just reads/writes them.
 *
...
class TagDatabase(context: Context) : SQLiteOpenHelper(context.applicationContext, DB_NAME, null, DB_VERSION) {
     ...
     *
     *  Bug fix ("typing numbers or symbols... shows no results"):
    ...
    /** Default "browse all" view for the Liked search tab, most
    ...
    /** The complete tag list for one post (for the Tags mode on a
     ...
     *
     *  [DatasetInfo.postCount] is computed live off liked_media rather than
     *  stored, so it can never drift out of sync with reality (e.g. if a
     *  future feature ever lets someone delete individual posts). */
     ...
     *  "Delete Tagged Post Database" button via
    ...
    fun clearAll() {
        ...
        // "Delete Tagged Post Database" is the nuclear "start over" option,
        // so it clears every imported dataset's listing too, not just the
        // local tagged rows — otherwise Settings' imported-datasets list
        // would keep showing entries whose underlying liked_media/media_tags
        // rows had already been wiped.
    ...
    companion object {
        ...
        fun get(): TagDatabase =
            instance ?: synchronized(this) {
                instance ?: TagDatabase(AndroidAppContext.app).also { instance = it }
            }
    }
}
