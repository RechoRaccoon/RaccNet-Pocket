 ...
 *  Runtime Mobile's own graph optimizations plus the XNNPACK execution
 *  provider (see OnnxImageTagger) for on-device speed — ConvNeXt at
 ...
 *  mid-range device) is fine for tagging a like as it
 *  happens or working through a liked-posts backlog in the background.
 *
 *  KMP port: the legacy OkHttpClient is replaced by Ktor (OkHttp engine),
 *  and the Context parameter is gone — the app-private model dir comes
 *  from [AndroidAppContext]. */
class TaggerModelManager {
    ...
    private val modelDir: File by lazy { File(AndroidAppContext.app.filesDir, "tagger").apply { mkdirs() } }
    ...
    private val httpClient = HttpClient(OkHttp) {
        install(HttpTimeout) {
            // No total request timeout: a ~390MB model on a slow link must
    ...
    private suspend fun downloadTo(url: String, dest: File, onProgress: (Long, Long) -> Unit) {
        ...
        if (!response.status.isSuccess()) error("HTTP ${response.status.value} fetching ${dest.name}")
