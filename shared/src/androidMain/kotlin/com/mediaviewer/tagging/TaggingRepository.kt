...
import com.mediaviewer.platform.PlatformImageTagger
import com.mediaviewer.platform.TaggingProgress
 ...
 *
 *  KMP port: the Context parameter is gone (see [AndroidAppContext]);
 *  OkHttp is Ktor (OkHttp engine); Coil 2 is Coil 3; and inference goes
 *  through [PlatformImageTagger] (whose tagImage takes encoded bytes, so
 *  bitmaps are PNG-encoded before inference — PNG is lossless, so the
 *  tagger sees pixel-identical input). */
class TaggingRepository(
    ...
    // (holding the application context explicitly avoids ever accidentally
    // leaking an Activity context into a longer-lived repository field).
    private val appContext = AndroidAppContext.app
    private val db = TagDatabase.get()
    private val modelManager = TaggerModelManager()
    private val platformTagger = PlatformImageTagger()
     ...
     *  — reads straight from the already-downloaded tags CSV, so
    ...
    /** Warms the platform tagger (downloading the model on first use),
     *  streaming 0..1 progress for the overlay. The legacy version surfaced
     *  per-file byte counts via TaggerModelManager.State; the shared
     *  [TaggingProgress] contract has no model-state field, so only the
     *  fraction is forwarded. */
    private suspend fun ensureTagger(onModelProgress: (Float) -> Unit) {
        if (!platformTagger.ensureReady(onModelProgress)) error("Model not ready")
    }

    fun cancel() { cancelRequested = true }

    /** Settings' "Delete Tagged Post Database" button: wipes every
     ...
     *  stopped first (the caller does this) so it doesn't keep writing rows
     *  back in while this runs. */
    suspend fun deleteDatabase() = withContext(Dispatchers.IO) { db.clearAll() }

    // ── Import/Export ─────────────────────────────────────────────────
    // JSON (de)serialization itself lives in the shared download/settings
    // UI layer (a file picked by the user is parsed there); this repository
    // only ever deals in plain Kotlin data (platform.ExportedPost/
    // DatasetInfo), same as every other method here.

    /** Everything currently tagged, across every dataset — the source data
     *  for Settings' "Export" button. */
    suspend fun exportAllPosts(): List<ExportedPost> = withContext(Dispatchers.IO) { db.allPostsForExport() }
    ...
    suspend fun listImportedDatasets(): List<DatasetInfo> = withContext(Dispatchers.IO) { db.listImportedDatasets() }
    ...
    suspend fun tagAllLiked(
        ...
        withContext(Dispatchers.IO) {
            fun progress(scanned: Int, tagged: Int, isRunning: Boolean, isComplete: Boolean, currentItem: MediaItem? = null) =
                TaggingProgress(scanned, tagged, db.datasetSizeBytes(), isRunning, isComplete, currentItem)

            onProgress(progress(db.scannedCount(), db.taggedCount(), isRunning = true, isComplete = false))
            try {
                // Model download progress has no dedicated field on the
                // shared progress type — re-emit the counters so the overlay
                // at least stays live while the ~390MB model downloads.
                ensureTagger { onProgress(progress(db.scannedCount(), db.taggedCount(), isRunning = true, isComplete = false)) }
            } catch (e: Exception) {
                onProgress(progress(db.scannedCount(), db.taggedCount(), isRunning = false, isComplete = false))
                return@withContext
            }
            ...
            // Tracks whichever post the single serial inference consumer
            // below is currently on, for reportProgress() to attach to every
            // update. An AtomicReference rather than a plain local var for
            // the same cross-thread-visibility reason scanned/tagged above
            // are AtomicIntegers — the consumer coroutine that writes it and
            // the producer coroutines that also call reportProgress() (for
            // text-only items) can run on different Dispatchers.IO threads.
            val currentItemRef = java.util.concurrent.atomic.AtomicReference<MediaItem?>(null)
            fun reportProgress() {
                onProgress(progress(scanned.get(), tagged.get(), isRunning = true, isComplete = false, currentItem = currentItemRef.get()))
            }

            /** Tags a batch of not-yet-indexed items.
             *
             *  A fetch/decode pipeline feeding one serial inference
             *  consumer. Concurrent `Run()` calls into the same shared
             *  `OrtSession` just queue/timeshare the same cores (its
             *  intra-op thread pool already uses nearly all of the phone's
             *  cores for a *single* inference) instead of actually speeding
             *  anything up — but fetch+decode genuinely parallelize
             *  (network I/O and bitmap decode aren't CPU-bound on the
             *  inference cores). So [parallelism] producer coroutines fetch+
             *  decode concurrently and push results onto a bounded channel,
             *  and exactly one consumer drains it and calls into the tagger
             *  serially — while post N is being inferred, N+1, N+2, ... are
             *  already being fetched/decoded in the background. The "posts
             *  tagged at once" slider controls how many items are
             *  prefetched ahead of the inference queue, rather than how many
             *  are inferred at the same instant (see the Settings copy for
             *  this slider). */
            suspend fun tagBatch(items: List<MediaItem>) {
                ...
                coroutineScope {
                    ...
                    for (prepared in preparedQueue) {
                        ...
                        // Report the post as "now tagging" before running
                        // inference on it (not just after), so the
                        // full-screen overlay shows it for the whole time
                        // it's actually being tagged.
                        currentItemRef.set(prepared.item)
                        reportProgress()
                        val newlyTagged = inferAndStore(prepared.item, prepared.media)
                        scanned.incrementAndGet()
            ...
            onProgress(progress(scanned.get(), tagged.get(), isRunning = false, isComplete = !cancelRequested))
        }
    }

    /** Realtime path: tags exactly one just-liked post, used by the
     *  MainViewModel.toggleLike equivalent when the "Tag Post When Liked"
     *  setting is on. Silently no-ops (rather than throwing) if the model
     *  isn't downloaded yet — realtime tagging only makes sense once the
     *  person has already run the initial "Locally Tag All Liked Posts"
     *  pass, at which point the model is guaranteed to already be on disk. */
    suspend fun tagOnLike(item: MediaItem) {
        if (!modelManager.isReady()) return
        withContext(Dispatchers.IO) {
            try { ensureTagger { } } catch (_: Exception) { return@withContext }
            tagOnePost(item)
        }
    }

    /** Tagging-speed fix: routed through Coil 3's shared ImageLoader (memory
     *  + disk cache) instead of always doing a fresh network fetch. For the
     ...
     *  hardware Bitmap can't be read back for pixel access.
     *
    ...
    private suspend fun fetchBitmapForTagging(url: String): Pair<Bitmap, Boolean>? {
        ...
        try {
            ...
            val request = ImageRequest.Builder(appContext)
                .data(url).size(Size(896, 896)).allowHardware(false).build()
            val image = (loader.execute(request) as? SuccessResult)?.image as? BitmapImage
            if (image != null) return image.bitmap to false
     ...
     *  ever touching the shared tagger session. */
     ...
     *  shared tagger session's intra-op thread pool already uses nearly all
    ...
    private suspend fun inferAndStore(item: MediaItem, prepared: PreparedMedia): Boolean {
        val finalBitmap = prepared.bitmap
        if (finalBitmap == null) {
            db.storeTags(item.postUri, item.postCid, prepared.sourceUrlForRecord, emptyList())
            return false
        }
        // PlatformImageTagger takes encoded bytes; PNG is lossless so the
        // tagger sees pixel-identical input to the bitmap above.
        val tags = try {
            platformTagger.tagImage(bitmapToPngBytes(finalBitmap))
                .map { it.name to it.confidence }
        } finally {
            if (prepared.ownsBitmap) finalBitmap.recycle()
        }
    ...
    private suspend fun tagOnePost(item: MediaItem): Boolean {
        if (item.postUri.isBlank()) return false
        if (item.isTextOnly) {
            db.storeTags(item.postUri, item.postCid, "", emptyList())
            return false
        }
        return inferAndStore(item, prepareMedia(item))
    }
     ...
     *  then again in the tagger's own resize, losing
     ...
     *  discard ~95% of it in the tagger's own downscale wastes real
    ...
    private suspend fun fetchBytes(url: String): ByteArray? = try {
        ...
        if (!response.status.isSuccess()) null else response.readBytes()
    } catch (_: Exception) { null }

    /** Search page's "Liked" tab.
     *
     *  Bug fix: a query that's non-blank but sanitizes away to
     ...
     *  blank query already browses everything tagged so far — a query with
     *  no *searchable* characters in it should behave the same way rather
     *  than looking "broken" with a permanent empty result. */
    ...
    /** Default view for the Liked tab: every tagged post, most
    ...
    /** Full tag list for one post — the Tags mode needs to display
    ...
    companion object {
        ...
        fun get(bskyRepo: BlueskyRepository, e621Repo: E621Repository): TaggingRepository =
            instance ?: synchronized(this) {
                instance ?: TaggingRepository(bskyRepo, e621Repo).also { instance = it }
            }
    }
}
