 ...
 * on top of it.
 *
 * Porting note: the signature is platform-free here (URL strings instead of
 * android.net.Uri) so the no-op lives in commonMain. If the Media3
 * implementation is ever restored, it becomes an expect/actual with the
 * stitching in androidMain only (there is no web upload path that needs
 * this).
 */
object VideoThumbnailStitcher {
    /** Returns [thumbnailUrl] spliced into [videoUrl] as its first frame —
     *  currently just returns [videoUrl] unchanged; see class doc above. */
    suspend fun stitch(videoUrl: String, thumbnailUrl: String?): String = videoUrl
}
