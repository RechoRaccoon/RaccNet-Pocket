 ...
 *  [customFontFamily] (built from a user-picked .ttf/.otf, see App.kt's
 *  loadFontFamily — Android-only) to every Material3 text style. Text()
 *  composables across the app never pass their own explicit fontFamily —
 *  they only ever override size/weight/color — so overriding it once here,
 *  on every style in Typography, is enough to reach every screen without
 *  touching each call site individually. Null (the default/no custom font
 *  picked) falls back to Typography()'s own system default exactly as
