...
private fun lerpHsv(from: Color, to: Color, t: Float): Color {
    ...
    // PORT: was android.graphics.Color.colorToHSV — reimplemented in pure
    // Kotlin below (same algorithm/semantics), since that API is Android-only.
    colorToHsv(from, fromHsv)
    colorToHsv(to, toHsv)
...
/** Pure-Kotlin equivalent of android.graphics.Color.colorToHSV (h in
 *  0..360, s/v in 0..1). */
private fun colorToHsv(c: Color, hsv: FloatArray) {
    ...
    hsv[0] = when {
        delta == 0f -> 0f
        max == c.red -> 60f * (((c.green - c.blue) / delta) % 6f)
        max == c.green -> 60f * ((c.blue - c.red) / delta + 2f)
        else -> 60f * ((c.red - c.green) / delta + 4f)
    }
    if (hsv[0] < 0f) hsv[0] += 360f
}

/** Pure-Kotlin equivalent of android.graphics.Color.HSVToColor. */
