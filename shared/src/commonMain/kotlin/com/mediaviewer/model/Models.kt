...
// KMP port of the Android app's model layer (Retrofit/Gson -> Ktor/kotlinx.serialization).
// Class/enum names and property shapes are kept identical to the Android source so
// repositories, viewmodels and UI can be ported against the same shapes.
// Intentional serialization-layer changes vs the Gson original:
//  - @Serializable on every DTO; @SerialName where Gson used @SerializedName.
//  - com.google.gson.JsonElement/JsonObject -> kotlinx.serialization.json equivalents.
//  - Map<String, Any> record/embed/facet payloads -> kotlinx JsonObject (Gson's
//    untyped maps are not serializable by kotlinx.serialization).
//  - BskyImageView.thumb/fullsize gained = "" defaults: Gson silently left missing
//    keys as runtime nulls (see the gallery-thumbnail comment on BskyImageView);
//    kotlinx.serialization would throw on a missing non-null key instead.
//  - LeafletBlog.blocks keeps @Transient (now kotlinx's) for the same on-disk
//    cache reason documented on the property.
...
data class BskySendMessageInput(
    val text: String,
    val facets: List<JsonObject>? = null,
