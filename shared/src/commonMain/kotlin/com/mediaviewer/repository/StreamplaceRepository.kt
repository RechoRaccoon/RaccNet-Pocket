...
class StreamplaceRepository {
    ...
    suspend fun getVods(handleOrDid: String, cursor: String? = null, limit: Int = 25)
        ...
        val body = apiCall("Streamplace video list") { api.getVideoList(repo = handleOrDid, limit = limit, cursor = cursor) }
    ...
    suspend fun getLiveFriends(friendDids: Set<String>): Result<List<StreamplaceLiveStream>> = runCatching {
        ...
        val body = apiCall("Streamplace live users") { api.getLiveUsers(limit = 100) }
        body.streams
    ...
    private fun StreamplaceRawLivestreamView.toLiveStream(): StreamplaceLiveStream {
        ...
        // (the sibling network/ port must make the same change on
        // StreamplaceRawLivestreamView.record).
        val title = record?.get("title").asStr() ?: ""
        val thumbCid = record?.get("thumb").asObj()?.get("ref").asObj()?.get("\$link").asStr()
