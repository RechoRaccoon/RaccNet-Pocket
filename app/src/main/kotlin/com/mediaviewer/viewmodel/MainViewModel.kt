...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    enum class ProfileTab { MEDIA, TEXT_POSTS, REPOSTS, LIKES, BLOGS, REVIEWS, BACKLOG }
...
_bskyDid.value          = session.did
    ...
    fun openProfile(author: AuthorInfo, initialTab: ProfileTab = ProfileTab.MEDIA) {
        if (!_bskyLoggedIn.value) return
        _profileOverlay.value = ProfileOverlayState(author = author, selectedTab = initialTab)
    ...
    private fun loadProfileTab(tab: ProfileTab, reset: Boolean) {
        // Blogs/Reviews/Backlog are fully loaded up-front by the probes in
        // openProfile() — there's no separate paged fetch for them.
        if (tab == ProfileTab.BLOGS || tab == ProfileTab.REVIEWS || tab == ProfileTab.BACKLOG) return
        val cur = _profileOverlay.value ?: return
